...
if ! buildah containers --format "{{.ContainerName}}" | grep -q gobuilder-core; then
    ...
    buildah from --name gobuilder-tmp docker.io/library/golang:1.25.2-bookworm
...
(
    cd "${logcli_tmp_dir}"
    curl -L -O https://github.com/grafana/loki/releases/download/v3.5.6/logcli-linux-amd64.zip
...
printf "PROMTAIL_IMAGE=docker.io/grafana/alloy:v1.9.1\n" >> "${core_env_file}"
...
container=$(buildah from docker.io/library/redis:7.2.11-alpine)
reponame="redis"
...
container=$(buildah from docker.io/library/alpine:3.22.1)
reponame="restic"
