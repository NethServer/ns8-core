...
if ! buildah containers --format "{{.ContainerName}}" | grep -q gobuilder-core; then
    ...
    buildah from --name gobuilder-tmp docker.io/library/golang:1.26.7-bookworm
...
(
    cd "${logcli_tmp_dir}"
    curl -L -O https://github.com/grafana/loki/releases/download/v3.6.16/logcli-linux-amd64.zip
...
container=$(buildah from docker.io/library/redis:8.8.2-alpine)
reponame="redis"
