...
if ! buildah containers --format "{{.ContainerName}}" | grep -q gobuilder-core; then
    ...
    buildah from --name gobuilder-tmp docker.io/library/golang:1.24.6-bookworm
...
(
    cd "${logcli_tmp_dir}"
    curl -L -O https://github.com/grafana/loki/releases/download/v3.5.4/logcli-linux-amd64.zip
