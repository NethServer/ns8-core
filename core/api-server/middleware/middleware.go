...
func InitJWT() *jwt.GinJWTMiddleware {
	...
	authMiddleware, errDefine := jwt.New(&jwt.GinJWTMiddleware{
		...
		Authenticator: func(c *gin.Context) (interface{}, error) {
			...
			otpNeed := methods.Check2FA(username)
			if otpNeed && otpValue == "" {
				return nil, errors.New("missing OTP value")
			}

			// OTP check result: true=success, false=not needed
			otpPass := false
			if otpNeed {
				userSecret := methods.GetUserSecret(username)
				// Precondition: user secret is stored in the disk
				if userSecret == "" {
					err := errors.New("Secret for OTP generation of user " + username + " was not found")
					utils.LogError(err)
					return nil, err
				}
				// Main condition: the OTP value was generated by the user secret
				if ! methods.CheckOTP(userSecret, otpValue) {
				...
				otpPass = true // claim that 2FA is enabled and used
			}
			...
			return &models.UserAuthorizations{
				Username: username,
				OtpPass: otpPass, // true if OTP passed, false if OTP is not needed
			}, nil
