...
- `user-domain-changed`: the user domain has become available, has been
  ...
  `{"node_id":INT,"domain":STRING,"domains":LIST[STRING]}`. Note: the `domain` attribute is deprecated but still included for backward compatibility. The `node_id` attribute is not
