...
|cluster/module_uuid                    |HASH       |Store the UUID of each module instance. To avoid UUID clashes, clone and restore may store a temporary UUID until the action finishes. |
...
|cluster/password_warning/templates|HASH     |Password warning templates, the hash contains 2 keys: "<domain>_custom_template_content" and "<domain>_custom_mail_subject|
